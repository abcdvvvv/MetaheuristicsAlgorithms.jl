...
"""
function ChOA(objfun, lb::Real, ub::Real, npop::Integer, max_iter::Integer, dim::Integer)
    return ChOA(objfun, fill(lb, dim), fill(ub, dim), npop, max_iter) 
end

function ChOA(objfun, lb::Vector{Float64}, ub::Vector{Float64}, npop::Integer, max_iter::Integer)
    dim = length(lb)
...
function ChOA(problem::OptimizationProblem, npop::Integer=30, max_iter::Integer=1000)::OptimizationResult
    return ChOA(problem.objfun, problem.lb, problem.ub, npop, max_iter)
end